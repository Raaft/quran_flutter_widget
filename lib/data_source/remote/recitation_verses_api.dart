...
class RecitationVersesApi {
  Future<MyResponse<RecitationVerses>> fetchRecitationVersesList(
    ...
    var response = await DioUtils.request(DioUtils.REQUEST_GET,
        "${Apis.FETCH_ALL_RECITATIONS}{$recitationId}/verses/");
    if (response != null) {
  ...
  Future<MyResponse<RecitationVerses>> fetchRecitationVersesById(
    ...
    var response = await DioUtils.request(DioUtils.REQUEST_GET,
        "${Apis.FETCH_ALL_RECITATIONS}{$recitationId}/verses/$recitationVersesId");
    if (response != null) {
      if (response.statusCode == 200) {
